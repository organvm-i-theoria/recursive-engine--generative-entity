...
class MemoryNode:
    ...
    def decay(self):
        """
        Evaluates decay state using unified charge tier system.
        See: RE-GE_CONSTANTS_CHARGE_THRESHOLDS.md
        - LATENT tier (0-25): Moves to latent state
        - Ritual-tagged: Always preserved
        """
        if self.charge <= 25:  # LATENT tier threshold
            self.status = "latent"
...
- FUSE01 → If multiple past versions overlap (See: RE-GE_PROTOCOL_FUSE01.md)
  - Overlap criteria: 3+ versions detected OR charge >= 70 with 2+ overlapping fragments
  - FUSION_REGISTRY.json stores all fusion logs
